...
- `scripts/articles.py` = `articles.md` 生成 + **記事ゲート**（`--check`）。スコアと記事リンクは分離: リンク未着は
  ⏳ **配信待ち**として surface するだけで公開を止めず、毎 run で再探索 backfill する（グローバルゲートで全公開を人質に取らない）。
  **blocking**（ref 欄欠落・未知 cat・URL 重複・実在しない結果を指す記事ブロック）のみ exit 1。`--selftest` 内蔵。
