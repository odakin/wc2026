...
- 全リンクは **FIFA公式マッチレポート記事** (`/articles/…`、ハイライト動画が埋め込まれた記事) に統一する方針
  （`/watch/` 動画頁・マッチセンター頁は使わない。日本語版優先・未配信は英語版へフォールバック。2026-06-23 確定）。
- 英語ページの FIFA リンクは `build.py` の `fifa_localize()` が決定論変換（日本語記事 URL = `/ja/`→`/en/` かつ
  語尾 `-ja` 除去 / 英語記事 URL はそのまま通る = ja/en とも同じ英語記事）。例外は ref の `url_en` で上書き。
...
2. `data/articles.yaml` に **FIFA公式マッチレポート記事**の URL を追記（実在確認してから）。
   ハイライト動画は記事に埋め込まれているので必ず記事 (`/articles/…`) にリンクし、
   **むき出しの動画頁 (`/watch/`) やマッチセンター (スタッツ頁) は使わない**。
   日本語版 `/ja/…-ja` を優先、未配信なら英語版 `/en/…/articles/<slug>` にフォールバック（`note` に明記）。
3. `python3 scripts/standings.py --write && python3 scripts/articles.py --write` でビューを再生成。
4. `python3 scripts/articles.py --check`（記事リンク欠落ゲート）→ `python3 build.py`（docs 再生成）。
