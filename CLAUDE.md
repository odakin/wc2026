...
- 英語ページの FIFA リンクは `build.py` の `fifa_localize()` が日本語 URL から決定論変換
  （記事 = `/ja/`→`/en/` かつ語尾 `-ja` 除去 / match-centre = `/ja/`→`/en/`）。例外は ref の `url_en` で上書き。
...
2. `data/articles.yaml` に FIFA レポート URL（日本語）を追記（実在確認してから）。
