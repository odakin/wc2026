...
- `scripts/check-result-finality.py` = **早すぎ確定ゲート**（新規 result が kickoff+105分 未満〔PK 決着は +145分 未満〕の記録なら exit 1 = 試合中ライブスコアの最終結果化を機械 backstop。「確定」の一次定義は AUTO_UPDATE.md §5。M76/M82 の 2 事故が起源。`--selftest` 内蔵）。
- `scripts/propagate-knockout.py` = **決勝T 対戦カード自動進行**（script 内蔵の `BRACKET_SLOTS` = ブラケット構造 SoT ⊕ results から R16 以降の label を毎回再導出。冪等 + **自己修復** = results の事後訂正が誤伝播済み label にも自動で届く〔2026-07-02 M82 誤記録→M94 セネガル焼き込み事故の再発防止〕。R32結果→R16→QF→… と 1 段ずつ。`--apply` で書込、既定 dry-run、`--selftest` 内蔵）。AUTO_UPDATE.md §6 が毎回実行。
