...
# 開催スタジアム → IANA タイムゾーン (en で venue local 時刻に変換するため)。
# 大会期間中 (6/11-7/20) は北米全域が DST 中、 メキシコは 2022 から CST 通年。
VENUE_TZ = {
    "AT&T Stadium": "America/Chicago",         # Arlington TX (Dallas)
    "Arrowhead Stadium": "America/Chicago",    # Kansas City MO
    "NRG Stadium": "America/Chicago",          # Houston TX
    "BC Place": "America/Vancouver",           # Vancouver BC
    "BMO Field": "America/Toronto",            # Toronto ON
    "Estadio Akron": "America/Mexico_City",    # Zapopan (Guadalajara) MX
    "Estadio Azteca": "America/Mexico_City",   # Mexico City MX
    "Estadio BBVA": "America/Mexico_City",     # Guadalupe (Monterrey) MX
    "Gillette Stadium": "America/New_York",    # Foxborough MA (Boston)
    "Hard Rock Stadium": "America/New_York",   # Miami Gardens FL
    "Lincoln Financial Field": "America/New_York",  # Philadelphia PA
    "Mercedes-Benz Stadium": "America/New_York",    # Atlanta GA
    "MetLife Stadium": "America/New_York",     # East Rutherford NJ
    "Levi's Stadium": "America/Los_Angeles",   # Santa Clara CA (SF)
    "Lumen Field": "America/Los_Angeles",      # Seattle WA
    "SoFi Stadium": "America/Los_Angeles",     # Inglewood CA (LA)
}

# チーム名 → FIFA 公式チームページ slug。 URL は https://www.fifa.com/{ja|en}/teams/<slug>。
# 値は FIFA 公式 API (cxm-api.fifa.com/fifaplusweb/api/teams/<slug>) で canonical と確認した
# 英語名 slug。 lowercase + ASCII hyphen。 unicode (côte / curaçao) は ASCII 化して使う。
# 多 word 国の hyphenation は未検証のものもあり (cape-verde / dr-congo 等)、 404 なら更新する。
TEAM_FIFA_CODE = {
    ...
    "スペイン": "spain", "ウルグアイ": "uruguay", "サウジアラビア": "saudi-arabia", "カーボベルデ": "cape-verde",
    "フランス": "france", "ノルウェー": "norway", "セネガル": "senegal", "イラク": "iraq",
    "アルゼンチン": "argentina", "オーストリア": "austria", "ヨルダン": "jordan", "アルジェリア": "algeria",
    "ポルトガル": "portugal", "コロンビア": "colombia", "ウズベキスタン": "uzbekistan", "コンゴ民主共和国": "dr-congo",
    "クロアチア": "croatia", "パナマ": "panama", "ガーナ": "ghana", "イングランド": "england",
}
...
def team_chip(t, lang, flag_side="left"):
    """flag + 短縮名を FIFA チーム紹介ページ link で wrap した HTML を返す。
    FIFA コード未登録のチームは link 無しの plain span にフォールバック (旗だけは出る)。
    flag_side="right" は links_card のアウェイ側 (名前→旗 の並び) 用。"""
    f_html = f'<span class="flag">{flag(t)}</span>'
    n_html = esc(short(t, lang))
    inner = (f_html + n_html) if flag_side == "left" else (n_html + f_html)
    code = TEAM_FIFA_CODE.get(t)
    if not code:
        return inner
    href = f"https://www.fifa.com/{lang}/teams/{code}"
...
STR = {
    "ja": {
        ...
        "nav": {"index.html": "クロス表", "standings.html": "順位表", "links.html": "記事リンク", "bracket.html": "決勝T"},
    ...
    "en": {
        ...
        "nav": {"index.html": "Cross-table", "standings.html": "Standings", "links.html": "Reports", "bracket.html": "Knockout"},
        ...
        "ko_intro": ("Full 32-match knockout bracket. Real teams fill each slot once groups are decided "
                     "(resolved group by group; unresolved slots keep their seed label). Times are local "
                     "to each venue (EDT / CDT / PDT / CST as labelled); scores / penalties appear after "
                     "each match. Scroll sideways for all rounds."),
...
.team-link:hover{text-decoration:underline;text-decoration-color:var(--muted);text-underline-offset:2px}
...
.ko-intro{color:var(--muted);font-size:14px;margin:6px 0 16px;max-width:76ch}
.ko-scroll{overflow:auto;padding-bottom:8px;-webkit-overflow-scrolling:touch}
.bracket{display:flex;align-items:stretch;min-width:max-content;min-height:1240px;padding:2px 2px 10px}
.ko-round{display:flex;flex-direction:column;width:174px}
.ko-round+.ko-round{margin-left:36px}
.ko-round>h3{font-size:12px;font-weight:800;color:var(--accent-ink);margin:0 0 6px;height:18px;
  display:flex;align-items:center;gap:7px;letter-spacing:.01em}
.ko-round>h3 .ko-cnt{color:var(--muted);font-weight:600;font-size:11px}
.ko-col{flex:1;display:flex;flex-direction:column}
.ko-match{flex:1;display:flex;flex-direction:column;justify-content:center;position:relative}
.ko-card{background:var(--panel);border:1px solid var(--line);border-radius:11px;padding:7px 9px;box-shadow:var(--shadow-sm)}
.ko-card.jp{border-color:var(--jp);box-shadow:inset 0 0 0 1px var(--jp),var(--shadow-sm)}
.ko-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:4px}
.ko-no{font-size:9.5px;font-weight:800;color:var(--muted);background:var(--line-soft);border-radius:5px;padding:1px 5px}
.ko-dt{font-size:10px;color:var(--muted);font-variant-numeric:tabular-nums}
.ko-team{display:flex;align-items:center;justify-content:space-between;gap:6px;font-size:12.5px;padding:2px 0}
.ko-team .kt{display:flex;align-items:center;gap:2px;min-width:0;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}
.ko-team.adv{font-weight:800}
.ko-team.adv .ko-sc{color:var(--win-ink)}
.ko-team.lbl .kt{color:var(--muted);font-size:11.5px}
...
.ko-round:not(:last-of-type) .ko-match:nth-child(even)::after{content:"";position:absolute;left:100%;top:50%;width:18px;height:2px;background:var(--line)}
...
def standings_card(g, rows, lang):
    ...
    for i, (t, s, tied) in enumerate(rows, 1):
        ...
        trs.append(
            ...
            f'<td class="team">{team_chip(t, lang)}</td>'
...
def links_card(g, matches, find_url, lang):
    ...
    for m in res:
        ...
        if url and "/watch/" in url:
            ...
            label, icon = L["report"], "📄"
        vs = (f'{team_chip(h, lang)}'
              f'<span class="sc">{m["hg"]}-{m["ag"]}</span>'
              f'{team_chip(a, lang, flag_side="right")}')
...
def ko_dt(iso, lang, venue=None):
    if not iso:
        return ""
    d = datetime.datetime.fromisoformat(iso)
    if lang == "ja":
        return f"{d.month}/{d.day}({JP_WD[d.weekday()]}) {d.hour:02d}:{d.minute:02d}"
    tz = VENUE_TZ.get(venue) if venue else None
    if tz:
        local = d.astimezone(zoneinfo.ZoneInfo(tz))
        return (f"{local.month}/{local.day} ({EN_WD[local.weekday()]}) "
                f"{local.hour:02d}:{local.minute:02d} {local.tzname()}")
    return f"{d.month}/{d.day} {d.hour:02d}:{d.minute:02d} JST"


def _ko_slot(slot, lang):
    if slot.get("team"):
        return team_chip(slot["team"], lang), True
...
def ko_match(m, lang):
    ...
    for side in ("home", "away"):
        ...
        scv = f'<span class="ko-sc">{sc["hg"] if side == "home" else sc["ag"]}</span>' if sc else ""
        cls = "ko-team" + (" adv" if m.get("winner") == side else "") + ("" if resolved else " lbl")
        rows.append(f'<div class="{cls}"><span class="kt">{txt}</span>{scv}</div>')
...
def third_race_section(race, lang):
    ...
    for r in race:
        ...
        trs.append(
            ...
            f'<td class="team">{team_chip(r["team"], lang)}</td>'
