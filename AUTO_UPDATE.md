...
- 期間: 大会の対象期間中のみ 30 分ごと。**停止日 = 2026-07-21**（決勝は現地 2026-07-19 だが 15:00 ET キックオフ = 結果は JST 07-20 朝に着地するため、07-20 中は最終結果の記録を継続し 07-21 に停止する）。
...
1. **期間チェック（最優先）**: 今が**停止日（2026-07-21）以降**なら、まず §2〜§7 を通常どおり実行して未記録の最終結果を記録してから、「終了」 セクションに従い停止する。
   それ以前（2026-07-20 まで）なら続行。
...
## 終了（対象期間後 = 2026-07-21 以降）

停止日（2026-07-21）以降の最初の実行で、最後に最終結果を反映してから、自分（この自動更新ジョブ）を停止する。
...
- **scheduled task 版**（旧経路）: この MCP に **delete tool は存在しない**ので、`update_scheduled_task` を taskId=`wc2026-auto-update`・enabled=false で呼んで無効化する（= 「削除」ではなく「無効化」が唯一の停止手段）。
