...
- **リンクは FIFA公式マッチレポート記事に統一**: ハイライト動画が埋め込まれた記事 (`/articles/…`) にリンクする
  （`/watch/` 動画頁・マッチセンター頁は使わない）。日本語版を優先し、未配信の試合は英語版記事にフォールバック。
- **日英リンクは決定論変換**: 日本語記事 URL は `build.py` の `fifa_localize()` が `/ja/`→`/en/` + 語尾 `-ja` 除去で
  英語ページ用に変換（英語記事 URL はそのまま）。例外は ref の `url_en` で上書き可。
