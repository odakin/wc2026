...
- **日英リンクは決定論変換**: `articles.yaml` には日本語 URL だけを記録し、英語ページの FIFA リンクは
  `build.py` の `fifa_localize()` が変換する（記事 = `/ja/`→`/en/` かつ語尾 `-ja` 除去 / match-centre =
  `/ja/`→`/en/`）。例外は ref の `url_en` で上書き可。
