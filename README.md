...
英語ページ（`/en/`）は同じデータから生成され、FIFA リンクも英語ページ向けに変換されます（日本語記事が未配信の試合は日英とも同じハイライト動画にリンク）。
...
- **リンクの選び方**（① ja 記事 → ② 動画 fallback / マッチセンター不使用 / en は `fifa_localize()` 決定論変換）の
  正本は [`data/articles.yaml`](data/articles.yaml) ヘッダ「リンク優先順位」 節。
...
python3 scripts/articles.py  --check   # 記事ゲート（blocking のみ exit 1、リンク未着で止めない。詳細は scripts/articles.py docstring）
...
4. 記事ゲート（`articles.py --check`、blocking のみ exit 1 / リンク未着では公開を止めない。詳細は [`scripts/articles.py`](scripts/articles.py) docstring）と leak gate を通してから commit + push（Pages が自動反映）
